...
<body>
	...
	<form action="expense/expenseUpdate" id="f1" method="post"
		class="form-horizontal">
		<input type="hidden" name="expenseId" id="expenseId"
			value="${expense.expenseId }" /> <input type="hidden"
		...
		<div class="row">
			...
			<div class="col-xs-7">
				<div class="form-group ">
					...
					<div class="col-xs-9 ">
						<textarea rows="10" class="form-control" name="expenseDesc"
							cols="" placeholder="请输入报销详情">${expense.expenseDesc }</textarea>
					</div>
				</div>
			</div>
		</div>
		...
		<table class="table" id="tab1">
			...
			<c:forEach items="${detailList }" var="detail">
				<tr>
					...
					<td>${detail.costId }<input type="hidden" name="costIds"
						value="${detail.costId }" />
					</td>
					<td>${detail.costName }</td>
					<td><input type='text' value='${detail.detailMoney }'
						id='money' name='detailMoneys' class='form-control'
						style='width: 100px'></td>
					<td><input type='text' name='detailDescs'
						value="${detail.detailDesc }" class='form-control'></td>
					...
					<div class="modal-body">
						<table class="table" id="tab2">
							...
							<c:forEach items="${costList }" var="cost">
								<tr>
									<td><input type="checkbox" id="costId"
										value="${cost.costId }" /></td>
									<td>${cost.costId }</td>
									<td>${cost.costName }</td>
									<td>${cost.costDesc }</td>
								</tr>
